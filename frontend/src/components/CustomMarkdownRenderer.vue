...
const renderInteractiveElements = async () => {
  ...
  imageWrappers.forEach((wrapper) => {
    ...
    const vnode = h(ElImage, {
      ...
      fit: "contain",
      style:
        "max-width: 100%; max-height: 300px; margin: 8px 0; border-radius: 4px; cursor: pointer;",
    });
    ...
    const popoverContent = h("div", { class: "stock-popover-content" }, [
      ...
      h("div", { class: "stock-price" }, [
        h(
          ...
          {
            class: "price-value",
            style: { color: stockData?.change >= 0 ? "#e34d59" : "#00a870" },
          },
  ...
  refTags.forEach((tag) => {
    ...
    const popoverContent = h("div", { class: "ref-popover-content" }, [
      ...
      refData?.url
        ? h("a", { href: refData.url, target: "_blank", class: "ref-link" }, "查看原文")
        : null,
    ]);
