<template>
  <div class="sector-detail-panel" v-if="sector">
...
watch(sector, async () => { await nextTick(); renderChart() })
onMounted(async () => { await nextTick(); renderChart() })
onBeforeUnmount(disposeChart)
</script>

<style scoped>
.sector-detail-panel { display: flex; flex-direction: column; gap: 16px; }

.detail-header { display: flex; align-items: center; justify-content: space-between; }
.sector-name { font-size: 18px; font-weight: 600; color: #303133; }
.sector-pct { font-size: 22px; font-weight: 600; }
...
.constituent-chart { width: 100%; }
.mini-chart { width: 100%; height: 120px; }

.constituent-title { font-size: 12px; font-weight: 600; color: #606266; margin-bottom: 8px; }
.constituent-list { display: flex; flex-direction: column; gap: 6px; }
.constituent-row { display: flex; justify-content: space-between; align-items: center; font-size: 12px; }
.c-name { color: #303133; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.c-pct { font-weight: 600; flex-shrink: 0; }
</style>
