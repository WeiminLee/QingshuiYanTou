...
import {
  LineChart, Flame, BarChart3, Network, FileSearch, FileText,
  Search, Building2, MessageCircle, PieChart, ListChecks,
  HelpCircle, Globe, Folder, FileEdit, Wrench,
...
export function isKlineData(result) {
  ...
  try {
    ...
    return Array.isArray(parsed?.data) && parsed.data.length > 0 && parsed.data[0]?.close !== undefined
  } catch {
    return false
  }
}
