  ...
  <div class="stock-detail">
    ...
    <el-card class="info-card">
      <div class="stock-info">
        ...
        <div class="info-right">
          ...
          <div v-if="stockScore" class="score-badge">
            <el-tag :type="scoreTagType" size="small">综合评分 {{ stockScore.total_score.toFixed(1) }}</el-tag>
          </div>
        </div>
      </div>
      ...
      <div v-if="stockScore" class="score-breakdown">
        <el-tag size="small" type="info">动量 {{ stockScore.breakdown.momentum_score.toFixed(0) }}</el-tag>
        <el-tag size="small" type="info">趋势 {{ stockScore.breakdown.trend_score.toFixed(0) }}</el-tag>
        <el-tag size="small" type="info">资金 {{ stockScore.breakdown.capital_score.toFixed(0) }}</el-tag>
        <el-tag size="small" type="info">概念 {{ stockScore.breakdown.concept_bonus.toFixed(1) }}</el-tag>
        <el-tag size="small" type="info">估值 {{ stockScore.breakdown.valuation_bonus.toFixed(0) }}</el-tag>
      </div>
...
const fetchQA = async () => {
  ...
  try {
    const res = await axios.get(`/api/v1/information/qa/${tsCode.value}`, { params: { limit: 10 } })
...
const fetchAnnouncements = async () => {
  try {
    const res = await axios.get(`/api/v1/information/announcements/${tsCode.value}`, { params: { limit: 20 } })
...
<style scoped>
.stock-detail { max-width: 1400px; margin: 0 auto; }
...
.info-left h2 { margin: 0; font-size: 20px; }
.info-left .industry { margin: 4px 0 0; color: #909399; }

.price-info { text-align: right; }
.current-price { font-size: 28px; font-weight: bold; margin-right: 10px; }
.pct-change { font-size: 16px; font-weight: 500; }
.pct-change.up { color: #f56c6c; }
.pct-change.down { color: #67c23a; }
.score-badge { margin-top: 4px; text-align: right; }

.score-breakdown { display: flex; gap: 6px; margin-top: 8px; flex-wrap: wrap; }
...
.concept-name { color: #606266; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.score-num { font-weight: 600; min-width: 36px; text-align: right; }
.score-num.score-high { color: #67c23a; }
.score-num.score-mid { color: #e6a23c; }
.score-num.score-low { color: #909399; }

/* Q&A */
.qa-item { font-size: 13px; }
.qa-q { color: #303133; margin-bottom: 4px; }
.qa-a { color: #606266; margin-bottom: 4px; }
.answer-text { font-size: 12px; }
.qa-signals { margin-top: 4px; }
</style>
