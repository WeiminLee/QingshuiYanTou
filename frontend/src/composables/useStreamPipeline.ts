...
export interface StreamPipelineCallbacks {
  ...
  onToolEnd?: (messageId: string, toolId: string, result: string, durationMs: number, preview?: string) => void
  ...
  function handleBufferedEvent(payloadType: string, payload: unknown): void {
    ...
    switch (payloadType) {
      ...
      case 'tool_called': {
        ...
        const toolId = p?.id as string || `tool-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`
      ...
      case 'task_failed': {
        ...
        const status = payloadType === 'task_failed'
          ? 'error'
          : payloadType === 'task_completed'
            ? 'done'
            : 'running'
        const result = (p?.result as string) || (p?.error as string) || (p?.status as string) || payloadType
