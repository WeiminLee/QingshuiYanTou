...
export const getStock = (tsCode) => apiClient.get(`/stocks/${tsCode}`);

export const getWatchlist = () => apiClient.get("/stocks/watchlist");

export const addToWatchlist = (tsCode, note = "") =>
  apiClient.post("/stocks/watchlist", { ts_code: tsCode, note });

export const removeFromWatchlist = (tsCode) => apiClient.delete(`/stocks/watchlist/${tsCode}`);
...
export const getCapitalFlow = (period = "D", limit = 20) => {
  return apiClient.get("/stocks/capital-flow", { params: { period, limit } }).catch((err) => {
