...
class TestToolCallCorrectness:
    ...
    def test_single_tool_call_emits_tool_called(self):
        ...
        mock_llm = MockLLM(
            responses=[
                AIMessage(
                    content="",
                    tool_calls=[{"id": "tc_1", "name": "get_kline", "args": {"ts_code": "300308.SZ"}}],
                ),
        ...
        assert len(tool_called_events) >= 1, f"未发射 tool_called 事件，实际事件：{[e[0] for e in emitted]}"
    ...
    def test_multiple_tool_calls_emit_multiple_events(self):
        ...
        tool_called_events = [e for e in emitted if e[0] == "tool_called"]
        assert len(tool_called_events) == 2, f"应发射 2 个 tool_called，实际：{len(tool_called_events)}"
...
class TestSSEEventFormat:
    ...
    def test_thinking_delta_format(self):
        ...
        assert len(thinking_events) >= 1, f"未发射 thinking_delta，实际事件：{[e[0] for e in emitted]}"
    ...
    def test_tool_result_format(self):
        ...
        assert len(tool_result_events) >= 1, f"未发射 tool_result，实际事件：{[e[0] for e in emitted]}"
    ...
    def test_tool_result_preview_is_meaningful(self):
        ...
        _, data = tool_result_events[0]
        # preview 应包含统计信息
        assert "120" in data["result"] or "条" in data["result"], f"K线 preview 应包含条数统计，实际：{data['result']}"
    ...
    def test_stream_end_format(self):
        ...
        assert len(stream_end_events) >= 1, f"未发射 stream_end，实际事件：{[e[0] for e in emitted]}"
...
class TestEventSequence:
    ...
    def test_full_react_cycle_event_order(self):
        ...
        # stream_end 应为最后一个事件
        assert event_types[-1] == "stream_end", f"stream_end 应为最后一个事件，实际最后事件：{event_types[-1]}"
...
class TestSSEEventMapping:
    ...
    def test_visible_map_covers_core_events(self):
        ...
        for event in core_events:
            assert event in _VISIBLE_MAP, f"_VISIBLE_MAP 应包含 {event}，实际：{list(_VISIBLE_MAP.keys())}"
...
class TestBuildPreviewIntegration:
    ...
    def test_kline_preview_in_tool_result(self):
        ...
        assert "120" in preview or "条" in preview, f"K线 preview 应包含条数，实际：{preview}"
    ...
    def test_tavily_search_preview_in_tool_result(self):
        ...
        mock_llm = MockLLM(
            responses=[
                AIMessage(
                    content="",
                    tool_calls=[{"id": "tc_1", "name": "tavily_search", "args": {"query": "光模块"}}],
                ),
                AIMessage(content="完成。"),
            ]
        )
        ...
        preview = tool_results[0][1]["result"]
        assert "2" in preview or "篇" in preview, f"搜索 preview 应包含文章数，实际：{preview}"

    def test_concept_hot_preview_in_tool_result(self):
        """get_concept_hot 工具结果 preview 包含板块数"""
        mock_llm = MockLLM(
            responses=[
                AIMessage(content="", tool_calls=[{"id": "tc_1", "name": "get_concept_hot", "args": {}}]),
                AIMessage(content="完成。"),
            ]
        )
        ...
        preview = tool_results[0][1]["result"]
        assert "20" in preview or "板块" in preview, f"热度 preview 应包含板块数，实际：{preview}"
    ...
    def test_stock_profile_preview_in_tool_result(self):
        ...
        preview = tool_results[0][1]["result"]
        assert "光通信" in preview or "主营业务" in preview, f"股票概况 preview 应包含主营业务关键词，实际：{preview}"
...
class TestRunLeadAgentE2E:
    ...
    async def _test_emits_correct_events(self):
        ...
        with (
            ...
            def mock_invoke(messages, **kwargs):
                ...
                if call_count[0] == 1:
                    return AIMessage(
                        content="",
                        tool_calls=[{"id": "tc_e2e", "name": "get_kline", "args": {"ts_code": "300308.SZ"}}],
                    )
        ...
        assert "reasoning_start" in event_types, f"应有 reasoning_start，实际事件：{event_types}"
        assert "stream_end" in event_types, f"应有 stream_end，实际事件：{event_types}"
    ...
    async def _test_clarification_intercept(self):
        ...
        # 验证拦截
        assert result.get("status") == "clarification_requested", f"应被澄清拦截，实际状态：{result.get('status')}"

        event_types = [e[0] for e in emitted]
        assert "clarification_request" in event_types, f"应有 clarification_request 事件，实际：{event_types}"
    ...
    def test_all_events_json_serializable(self):
        ...
        for event_type, data in emitted:
            try:
                ...
                pytest.fail(f"事件 {event_type} 无法 JSON 序列化：{e}\ndata: {data}")
