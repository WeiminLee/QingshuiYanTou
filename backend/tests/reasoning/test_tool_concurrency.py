...
class TestToolExecutorFactory:
    ...
    def test_safe_to_parallel_includes_readonly_tools(self):
        ...
        assert expected.issubset(SAFE_TO_PARALLEL), f"缺少只读工具: {expected - SAFE_TO_PARALLEL}"
...
class TestToolConcurrencyIntegration:
    ...
    def test_client_has_concurrent_execution_logic(self):
        ...
        src = inspect.getsource(client)
        has_concurrency = (
            "asyncio.gather" in src or "to_thread" in src or "ThreadPoolExecutor" in src or "concurrent" in src
        )
        assert has_concurrency, "client.py 未包含并发执行逻辑"
...
class TestConcurrentExecutionBehavior:
    ...
    def test_parallel_execution_reduces_total_time(self):
        ...
        async def run_parallel(tools: list) -> float:
            ...
            results = await asyncio.gather(*[fake_tool(name, 0.1) for name in tools])
        ...
        assert parallel_time < 0.25, f"并发执行时间({parallel_time:.3f}s)应远小于串行(0.3s)"
