...
class TestGetMemoryContext:
    ...
    async def test_loads_from_mongodb_with_thread_id(self):
        ...
        with patch(
            ...
            await get_memory_context_async(_THREAD_ID, analyst_id=_ANALYST_ID)
...
class TestClientKgAnchorsBug:
    ...
    def test_client_no_harness_reference_before_definition(self):
        ...
        # 找定义行
        define_lines = [i for i, line in enumerate(lines, start=1) if "harness: HarnessManager | None = None" in line]
        ...
        assert not bad_refs, f"harness 在第 {first_def} 行定义，但被提前引用: {bad_refs}"
...
class TestClientRunLeadAgentMemoryFlow:
    ...
    async def test_loads_memory_before_agent_creation(self):
        ...
        with (
            ...
            patch("app.reasoning.langchain_agent.client._ensure_agent") as mock_agent,
            patch(
                "app.reasoning.langchain_agent.client._build_agent_config",
                return_value={"configurable": {}},
            ),
            patch.object(_client_mod, "_create_chat_model"),
            patch.object(
                _client_mod,
            ...
            patch.object(_client_mod, "check_clarification", return_value=None),
        ):
            ...
            assert "光模块行业" in system_prompt, "memory_content 未注入到 system_prompt"
    ...
    async def test_memory_loaded_with_correct_thread_id(self):
        ...
        with (
            ...
            patch.object(_client_mod, "_load_memory_context", capture),
            patch.object(_client_mod, "_create_chat_model"),
            patch.object(_client_mod, "check_clarification", return_value=None),
        ):
            mock_agent.return_value = (_make_mock_agent([]), None)
            ...
            assert _THREAD_ID in captured, f"memory 未使用请求的 thread_id={_THREAD_ID}，实际捕获: {captured}"
...
class TestKgAnchorsInjection:
    ...
    async def test_kg_anchors_passed_to_prompt_when_enabled(self):
        ...
        with (
            ...
            args, kwargs = mock_agent.call_args
            system_prompt = kwargs.get("system_prompt") or ""
            assert "中际旭创" in system_prompt, "KG Anchors 未注入 system_prompt"
    ...
    async def test_kg_anchors_absent_when_disabled(self):
        ...
        with (
            ...
            assert "Company）被提及" not in system_prompt, "KG Anchors 不应在 kg_anchors_enabled=False 时注入"
