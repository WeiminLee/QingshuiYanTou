...
from app.knowledge.evidence import JOB_COMBINED, JOB_VECTOR, STATUS_DONE, STATUS_FAILED, STATUS_PENDING
...
class FakeService:
    ...
    async def update_evidence_status(self, evidence_id: str, job_type: str, status: str, extractor_version: str = "evidence-v1") -> None:
...
def _worker(service=None) -> EvidenceExtractionWorker:
    return EvidenceExtractionWorker(service=service or FakeService(), batch_size=2, max_concurrency=2, worker_id="test-worker")
...
def test_run_once_limit_zero_returns_zero() -> None:
    async def main():
        ...
        assert result == {"claimed": 0, "success": 0, "failed": 0, "skipped": 0, "job_type": "combined"}
    asyncio.run(main())
...
def test_successful_combined_job_marks_done_and_updates_evidence() -> None:
    async def main():
        service = FakeService()
        service.jobs = [{"job_id": "J1", "evidence_id": "EV:1", "job_type": JOB_COMBINED, "status": STATUS_PENDING}]
...
def test_missing_evidence_marks_job_failed() -> None:
    async def main():
        service = FakeService()
        service.jobs = [{"job_id": "J1", "evidence_id": "EV:missing", "job_type": JOB_COMBINED, "status": STATUS_PENDING}]
        worker = _worker(service)
        result = await worker.run_once(limit=1, job_type=JOB_COMBINED)
...
def test_two_jobs_processed() -> None:
    async def main():
        service = FakeService()
        service.jobs = [
            {"job_id": "J1", "evidence_id": "EV:1", "job_type": JOB_COMBINED, "status": STATUS_PENDING},
            {"job_id": "J2", "evidence_id": "EV:2", "job_type": JOB_COMBINED, "status": STATUS_PENDING},
        ]
        worker = _worker(service)
