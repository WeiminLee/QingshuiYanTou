...
async def check():
    async with async_session() as sess:
        ...
        print('=== 光通信核心个股评分 ===')
        for r in rows:
            print(f'{r[1]:10s} | 总分={str(r[2]):6s} | 动量={str(r[3]):5s} | 资金={str(r[4]):5s} | 概念溢价={str(r[5]):5s} | 趋势={str(r[6]):5s} | {r[0]}')

        # 光通信相关概念评分
        result2 = await sess.execute(text('''
        ...
        print('=== 光通信相关概念评分 ===')
        for r in rows2:
            print(f'{r[1]:15s} | score={r[2]:5.2f} | 1日={r[3]:5.2f}% | 广度={r[4]:.3f} | 相对强度={r[5]:5.2f} | {r[0]}')

        # StockPool
        result3 = await sess.execute(text('''
        ...
        # 光通信个股是否在 StockPool
        optical_names = ['中际旭创','新易盛','光迅科技','天孚通信','源杰科技',
                         '仕佳光子','长光华芯','德科立','光库科技','剑桥科技',
                         '华工科技','亨通光电','长飞光纤','永鼎股份','中天科技',
                         '烽火通信','罗博特科','华懋科技','腾景科技','太辰光',
                         '凌云光','大族激光','中兴通讯']
        result4 = await sess.execute(text('''
