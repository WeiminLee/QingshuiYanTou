...
async def _advance_watermark(session, subject_ts_code: str, dimension: str, dimension_scope: str | None, level) -> dict:
    """水位线推进（只升不降）。NULL scope 不参与 ON CONFLICT 匹配，故走 select + insert/update。"""
    ...
    scope_cond = (
        Watermark.dimension_scope == dimension_scope
    ...
    if wm is None:
        ...
        if wm.max_level is None or level > wm.max_level:
            wm.max_level = level
            wm.first_reached_at = now
        wm.last_updated_at = now
    return _watermark_dict(wm)
