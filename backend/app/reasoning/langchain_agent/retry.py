"""
Retry 模块 — 参考 hermes-agent agent/retry_utils.py
...
DeerFlow 参考：
- hermes-agent 用 jittered_backoff 防止 thundering herd
    ...
    async def execute(self, func: Callable[..., T]) -> T:
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                logger.warning(
                    f"[Retry] attempt {attempt}/{self.max_attempts} failed: {e}. "
                    f"Retrying in {delay:.2f}s..."
                )
                await asyncio.sleep(delay)

        # 理论上不会到达这里（max_attempts 内会 raise）
        if last_exc:
            raise last_exc
