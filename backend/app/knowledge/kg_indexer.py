...
def check_extracted(ann_id: Optional[str] = None, doc_id: Optional[str] = None) -> Optional[dict]:
    """
    查询文档是否已提取。
...
def mark_done(
    ...
    investment_signal: Optional[dict] = None,
    ...
    update: dict = {
        "$set": {
            ...
            # ── 统计字段 ──────────────────────────────
            "entities_count": (
                (kg_result or {}).get("entities_created", 0)
                + (kg_result or {}).get("entities_updated", 0)
            ),
            "relations_count": (
                (kg_result or {}).get("relations_created", 0)
                + (kg_result or {}).get("relations_updated", 0)
            ),
